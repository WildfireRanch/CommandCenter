...
-- Purpose: User Preferences & Voltage-Based Decision System
-- Version: 1.9.0
-- Date: 2025-10-17
-- =====================================================
...
CREATE INDEX IF NOT EXISTS idx_miner_profiles_user ON miner_profiles(user_id);
CREATE INDEX IF NOT EXISTS idx_miner_profiles_priority ON miner_profiles(user_id, priority_level, enabled);
...
CREATE INDEX IF NOT EXISTS idx_hvac_zones_user ON hvac_zones(user_id);
...
SELECT 'V1.9 Migration completed successfully!' AS status;

SELECT '=== USERS ===' AS section;
SELECT email, role, created_at FROM users;

SELECT '=== USER PREFERENCES ===' AS section;
SELECT
    voltage_at_0_percent, voltage_at_100_percent,
...
SELECT '=== MINER PROFILES ===' AS section;
SELECT
    name, priority_level, power_draw_watts,
    start_voltage, stop_voltage,
    require_excess_solar, enabled
FROM miner_profiles
ORDER BY priority_level;

SELECT '=== HVAC ZONES ===' AS section;
SELECT
    zone_name, temp_too_hot, temp_too_cold,
    exhaust_fan_enabled, heating_enabled
FROM hvac_zones;
